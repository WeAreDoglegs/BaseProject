...
import com.doglegs.core.base.ABaseActivity;
...
public abstract class DogLegsBaseActivity<T extends RxPresenter> extends ABaseActivity<T> {
    ...
    protected void onResume() {
        super.onResume();
        handler.postDelayed(() -> uploadDeviceOperate(true, 1), 2000);
    }
    ...
    public void onBackPressed() {
        if (this instanceof SplashActivity /*|| this instanceof HomeActivity || this instanceof LoginRegisteredActivity*/) {
    ...
    public void handleDefaultEvent(RxEvent event) {
        if (event.getEventType() == RxEvent.EventType.SYSTEM_LOGIN_OUT) {
            ...
            finish();
        } else if (event.getEventType() == RxEvent.EventType.SYSTEM_CLOSE_ACTIVITY) {
